    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        ElseIf cmbdata.Text = "oldreceipt" Then
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                ob.Execute("Insert Into Acmain(Cid, Year_id, Department, Billtype, Billno, Billdate, PartyId, Acid, Remarks,Basic,Roundoff, Netamt,Receiptamt,ptype,tid,clid,cbj) Values(" & clsVariables.CompnyId & ",'" & dt.Rows(i).Item("finyear") & "','OMANGNARECEIPT'," & aq(dt.Rows(i).Item("cbj")) & "," & Val(dt.Rows(i).Item("tranno")) & ",'" & ob.DateConversion(dt.Rows(i).Item("transdate")) & "'," & Val(dt.Rows(i).Item("partyaccountid")) & "," & Val(dt.Rows(i).Item("purchaseaccountid")) & ",N'-'," & Val(dt.Rows(i).Item("dueamt")) & "," & Val(dt.Rows(i).Item("paneltyamt")) & "," & Val(dt.Rows(i).Item("Receiptamt")) & "," & Val(dt.Rows(i).Item("Receiptamt")) & ",'OReceipt'," & Val(dt.Rows(i).Item("columnid")) & ",1," & Val(dt.Rows(i).Item("cbjaccountid")) & ")", ob.getconnection())
