...
Public Class JVEntry
    Private Sub JVEntry_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Dramount_KeyDown(sender As Object, e As KeyEventArgs) Handles Dramount.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            dg.Rows(dg.Rows.Count - 1).Cells(3).Value = Val(CrAmount.Text)
            dg.Rows(dg.Rows.Count - 1).Cells(4).Value = Val(Dramount.Text)
    ...
    Private Sub ButSave_Click(sender As Object, e As EventArgs) Handles ButSave.Click
        ...
        If (sdate) >= gFinYearBegin And (sdate) <= gFinYearEnd Then
            If Val(ddr.Text) = Val(dcr.Text) Then
                ...
                For i As Integer = 0 To dg.Rows.Count - 1
                    ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate, Acid, ACName, Cramt,Remarks,Dramt,ptype) Values(" & clsVariables.CompnyId & ",'" & clsVariables.WorkingYear & "','Transfer'," & Docno.Text & ",'" & ob.DateConversion(Ddate.Text) & "'," & dg.Rows(i).Cells(0).Tag & ",N'" & dg.Rows(i).Cells(0).Value & "'," & dg.Rows(i).Cells(3).Value & ",N'" & Trim(dg.Rows(i).Cells(2).Value) & "'," & dg.Rows(i).Cells(4).Value & ",'Transfer')", ob.getconnection())
                    ob.Execute("Insert Into Acmain(Cid, Year_id, Department, Billtype, Billno, Billdate, PartyId, Acid,Receiptamt,Paymentamt,Remarks,ptype) Values(" & clsVariables.CompnyId & ",'" & clsVariables.WorkingYear & "','Transfer','Transfer'," & Val(Docno.Text) & ",'" & ob.DateConversion(Ddate.Text) & "'," & Val(dg.Rows(i).Cells(1).Tag) & "," & Val(dg.Rows(i).Cells(0).Tag) & "," & Val(dg.Rows(i).Cells(3).Value) & "," & Val(dg.Rows(i).Cells(4).Value) & ",N'" & Trim(dg.Rows(i).Cells(2).Value) & "','Transfer')", ob.getconnection())
                Next
                If dg.Rows.Count > 0 Then
                    dg.Rows.Clear()
                End If
